...
export function getSpaceValues(space: Space, key: string): string[] {
  switch (key) {
    case "space_kind":
      return space.space_kind ? [space.space_kind] : [];
    case "arbetssatt":
    case "intent":
      return space.intent ?? [];
    case "noise":
    ...
    case "facility":
      return space.facilities ?? [];
    case "lokaltyp":
      return space.lokaltyp ?? [];
...
export function resolveIcon(
  opt: Pick<FilterOption, "icon_url" | "default_icon">,
): { type: "lucide"; Icon: LucideIcon } | { type: "url"; url: string } | null {
